...
def get_optimal_partition(forest: set[Node]) -> tuple[set[Node], set[Node]]:
    pass
...
def continuous_sync(g: DiGraph) -> DiGraph:
    ...
    for node in strata_sort(g):
        ...
        upper, lower = get_optimal_partition(forest)
        up, down = get_up_and_down(upper, lower, depth_map, max_depth)
