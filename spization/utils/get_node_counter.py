...
@multimethod
def get_node_counter(node: Node) -> Counter[Node, int]:
    return Counter({node: 1})


@multimethod
def get_node_counter(parallel: Parallel) -> Counter[Node, int]:
    return sum((get_node_counter(child) for child in parallel), start=Counter())


@multimethod
def get_node_counter(serial: Serial) -> Counter[Node, int]:
